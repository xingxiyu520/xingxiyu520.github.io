import { useCallback, useEffect, useMemo, useState, type ClipboardEvent, type Dispatch, type DragEvent, type FormEvent, type KeyboardEvent, type SetStateAction } from 'react';
...
type AdminTab = 'dashboard' | 'articles' | 'projects' | 'friends' | 'shares' | 'media' | 'taxonomy' | 'analytics' | 'site' | 'password';
...
function sanitizePreviewHtml(html: string) {
  ...
  template.content.querySelectorAll('*').forEach((element) => {
    Array.from(element.attributes).forEach((attribute) => {
      ...
      if ((name === 'src' || name === 'href') && value.startsWith('data:') && !value.startsWith('data:image/')) {
        element.removeAttribute(attribute.name);
      }
    });
  });
...
function renderMarkdownPreview(markdown: string) {
  const html = marked.parse(markdown || '还没有正文。', {
    async: false,
    breaks: true,
    gfm: true,
  }) as string;
...
function AdminApp() {
  ...
  const [articleSavedSnapshot, setArticleSavedSnapshot] = useState(() => JSON.stringify(emptyArticleForm));
  const articleDirty = JSON.stringify(articleForm) !== articleSavedSnapshot;
  ...
  const confirmDiscardArticleChanges = useCallback(() => (
    !articleDirty || window.confirm('文章有未保存修改，确认离开吗？')
  ), [articleDirty]);
  ...
  const handleArticleSubmit = (event: FormEvent) => {
    ...
    void runAction(articleForm.id ? '文章已更新' : '文章已创建', async () => {
      let saved: ApiArticle;
      if (articleForm.id) {
        saved = await updateArticle(articleForm.id, payload);
      } else {
        saved = await createArticle(payload);
      }
      const savedForm = articleToForm(saved);
      setArticleForm(savedForm);
      setArticleSavedSnapshot(JSON.stringify(savedForm));
    });
  };
  ...
  const handleProjectSubmit = (event: FormEvent) => {
    ...
    const payload: ProjectPayload = {
      ...
      content_markdown: projectForm.content_markdown,
        ...
        <nav className="admin-tabs">
          {adminTabs.map((tab) => (
            <button
              ...
              onClick={() => {
                if (activeTab === 'articles' && tab.key !== 'articles' && !confirmDiscardArticleChanges()) {
                  return;
                }
      ...
      <section className="admin-main">
        ...
        {activeTab === 'articles' && (
          <ArticlesView
            ...
            onImport={(file) => {
              if (!confirmDiscardArticleChanges()) {
                return;
              }
              void runAction('Markdown 已导入', async () => {
                const imported = await importMarkdownArticle(file, articleForm.category_name || '默认');
                const importedForm = articleToForm(imported);
                setArticleForm(importedForm);
                setArticleSavedSnapshot(JSON.stringify(importedForm));
              });
            }}
            onEdit={(article) => {
              if (!confirmDiscardArticleChanges()) {
              ...
              setArticleSavedSnapshot(JSON.stringify(nextForm));
            }}
            onClear={() => {
              if (!confirmDiscardArticleChanges()) {
                return;
              ...
              setArticleSavedSnapshot(JSON.stringify(emptyArticleForm));
            }}
...
function ArticlesView({
  ...
  const previewHtml = useMemo(() => renderMarkdownPreview(form.content_markdown), [form.content_markdown]);
    ...
    <section className="admin-work-grid">
      <article className="admin-card admin-form-card">
        <div className="admin-card-head">
          ...
          <button type="button" className="admin-ghost-button" onClick={onClear}>清空</button>
        </div>
          ...
          <Field label="Markdown 正文">
            <div className="admin-editor-toolbar">
              <div className="admin-segmented">
                <button type="button" className={editorMode === 'edit' ? 'active' : undefined} onClick={() => setEditorMode('edit')}>编辑</button>
                <button type="button" className={editorMode === 'preview' ? 'active' : undefined} onClick={() => setEditorMode('preview')}>预览</button>
              </div>
              <span className={isDirty ? 'admin-save-state dirty' : 'admin-save-state'}>{isDirty ? '未保存' : '已保存'}</span>
            </div>
            {editorMode === 'edit' ? (
              <textarea
                ...
                rows={14}
                placeholder="支持粘贴/拖拽图片，上传后会自动插入 Markdown 图片语法。Ctrl+S 可保存。"
              />
            ) : (
              <div className="admin-markdown-preview" dangerouslySetInnerHTML={{ __html: previewHtml }} />
            )}
          </Field>
    ...
    <section className="admin-work-grid">
      <article className="admin-card admin-form-card">
        ...
        <form className="admin-form" onSubmit={onSubmit}>
          ...
          <Field label="项目详情 Markdown"><textarea className="admin-markdown-editor compact" value={form.content_markdown} onChange={(event) => updateFormField(setForm, 'content_markdown', event.currentTarget.value)} rows={8} /></Field>
